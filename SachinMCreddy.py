...
class major:
    ...
    def add_course(self, flag, course):
        if flag == 'E':
            ...
            raise ValueError( "value error forund unexpected flag ")
...
def file_read(path, num_fields, seperator=','or'|'or'*', header=False):
    try:
        ...
        raise FileNotFoundError(" cant find file : ", path)
    else:
        with fp:
            for n, line in enumerate(fp):
                if n == 0 and header:
                    ...
                    if len(fields) != num_fields:
                        raise ValueError("number of fields in the file is not equal ")
                    else:
                        yield fields
